...
export class ImportRulesComponent {
    ...
    import() {
        ...
        if (this.replace) {
            ...
            this.categoryRuleService.replaceRules(rulesToAdd).then(() => {
                this.snackBar.open("replaced all existing category rules with " + rulesToAdd.length + " new rules", "", { duration: 3000 });
                this.dialogRef.close();
            })
        } else {
            this.categoryRuleService.addRules(rulesToAdd).then(() => {
